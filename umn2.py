...
def MudarG():
    st.session_state.Nav = 'Cadastro para Voluntáriosa'
def MudarH():
...
if pagina == "Cadastro para Voluntários":
    ...
    with st.form(key="form_voluntario"):
        ...
        contato = st.text_input("Contato (Telefone ou E-mail)", max_chars=100)
        interesse = st.text_area("Interesse (ex: Tipo de atividades que gostaria de participar)")

        # Botão para submeter o cadastro
        submit_button = st.form_submit_button(label="Cadastrar Voluntário")

        if submit_button:
            # Verificação de campos obrigatórios
            if not nome or not contato or not interesse:
                st.error("Por favor, preencha todos os campos obrigatórios!")
            else:
                try:
    ...
    # Adiciona botão para download da lista de voluntários cadastrados
    st.subheader("Baixar Lista de Voluntários")
    try:
        df_voluntarios = pd.read_csv("voluntarios.csv")
        st.download_button(
            label="Baixar CSV",
            data=df_voluntarios.to_csv(index=False),
            file_name="voluntarios.csv",
            mime="text/csv"
        )
    except FileNotFoundError:
        st.warning("Nenhum voluntário cadastrado ainda.")
